...
- **[優化] 系統日誌終端機卡片視覺整合**：重構 `dashboard.html` 與 `index.css`，將 System Logs 終端機卡片改為與全站風格一致之毛玻璃擬態面板，移除內部區域的原生純黑及深灰底色，並將輸入欄與篩選框優化為全站統一的 `.t-input` 樣式，徹底消除原有的多層框線堆疊感，提升視覺一致性與科技感。

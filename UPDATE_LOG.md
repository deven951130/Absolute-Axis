...
- **[修復] 多版本核心動態切換（DeceasedCraft 1.20.1 / 你好新蒸程 1.21.1）**：解決當使用者在網頁切換至不同 Minecraft 版本的模組包時，底層啟動腳本與 libraries 未自動更換導致的反覆崩潰。後端 `_deploy_pack_to_lxc()` 在解壓重整完畢後，加入「動態探測模組版本並生成 `run.sh`」機制，自動偵測模組版本並產生對應的核心啟動引導檔，實現真正的跨版本無縫切換。我們已手動於 LXC 容器內安裝 DeceasedCraft 所需的 1.20.1 Forge 核心並順利啟動伺服器。
...
- **[修復] 中文世界資料夾與 Java 崩潰防範**：修正舊版本使用中文模組包檔名（如「你好新旅程」）進行 slug 轉換時包含中文字元，導致 Java (nio) 在載入世界路徑時丟出 `InvalidPathException` 而重啟死循環的 bug。透過 `_pack_slug()` 重構過濾規則，移除非 ASCII 字元，僅保留英數字、點與減號，並附加 MD5 唯一特徵值確保無損唯一性，並手動完成存檔遷移。
...
- **[修復] 模組包目錄多層嵌套與中文檔名解壓亂碼問題**：為解決 ZIP 解壓後因為中文檔名變成 Unicode 十六進位編碼名稱（如 `#U4f60#U597d...`）或多層嵌套，導致 `mods` 及 `config` 未能處於正確位置的問題，於部署解壓後加入「自動重整目錄結構」機制。藉由 awk 依路徑深度尋找最淺層的 `mods` 資料夾並將其所有內容搬移至根目錄下，同時清理臨時資料夾，保證伺服器能順利讀取模組順利開機。
...
- **[修復] 獨立介紹頁網站公網 Funnel 通道開通與自啟動配置**：
  - 於 Proxmox 宿主機 (PVE) 啟動先前處於停止狀態之獨立介紹頁容器 `axis-intro-lxc` (VMID 103) 並設定開機自動啟動（`onboot: 1`）。
...
#### 側邊欄模組化功能控制設定（P0）
- **[新增] 側邊欄功能可自訂勾選隱藏**：於「系統設定」介面新增「模組化功能控制」選項。使用者可個別勾選「接案功能」、「開源專案分享」與「Minecraft 伺服器」是否於側邊欄顯示。
- **[新增] 前端路由與路徑存取防禦**：當特定功能未勾選時，不僅側邊欄對應項目會自動隱藏，即便在網址列直接輸入該功能之 URL 路徑（如 `/gigs`、`/code-share`、`/multiverse`）進行存取，系統亦會強制攔截引導回首頁（`/main`）並自動修復瀏覽器歷史紀錄。

#### 多重宇宙 Minecraft 模組包管理與簡介展示（P0）
- **[新增] 模組包上傳與部署系統**：後端實作 `/api/minecraft/upload-server`、`/api/minecraft/upload-client` 與 `/api/minecraft/uninstall-server` 等端點。管理員可於前端直接上傳伺服器端包與客戶端包，系統會自動藉由 SFTP 傳輸至 Minecraft 容器並安全解壓部署（解壓時僅清除 mods、config 等目錄，完整保留 world 世界存檔）。
- **[新增] 模組包簡介編輯與展示區塊**：前端新增「模組包簡介」與「下載/管理」雙區塊。一般使用者可即時查閱簡介並一鍵下載客戶端模組包；管理員則可即時點擊編輯儲存簡介，簡介內容以 JSON 形式持久化於主機。

#### 接案中樞管理員接案管理功能（P0）
- **[新增] 管理員強制刪除/管理案件功能**：修改 `static/js/gigs.js`。當使用者以管理員身分登入時，在所有案件狀態（開放承接、進行中、已完成）下皆會額外呈現「刪除案件」按鈕，使管理員可一鍵物理刪除並稽核任何有爭議或違規之委託案件，全面提昇平台治理能力。

#### 開源專案分享平台管理員專案管理功能（P0）
...
- **[新增] 管理員自訂開源作者資訊設定**：於「系統設定 -> 模組化功能控制」中，新增管理員專屬的「開源專案平台資訊設定」區塊。管理員可隨時自訂「開發者 GitHub 帳號名稱」與「開發者 GitHub 主頁連結」，設定值會持久化儲存於後端 JSON 檔案中。
- **[新增] 動態作者資訊渲染與快取自動重建**：開源專案分享頁面會動態載入此設定進行渲染，使副標題與「造訪開發者 GitHub」連結按鈕隨設定而變。若管理員變更作者帳號名稱，後端亦會自動清除專案快取，下次載入時自動拉取新作者帳號的全部公開專案。
- **[新增] 管理員自訂 GitHub 存取權杖 (GITHUB_TOKEN) 排除 403 限制**：為了解決匿名請求 GitHub API 導致的 403 Rate Limit 錯誤，於「開源專案平台資訊設定」中新增「GitHub 個人存取權杖」密碼輸入框。設定值將以遮罩 `••••••••••••••••` 顯示，儲存後系統將自動優先套用此 Token 發送 API 請求，大幅提高頻率限制。

#### 問題反饋功能佈局優化（P0）
- **[優化] 移動意見反饋至網站底部左下角**：移除側邊欄選單中間的「意見反饋」選項。在左側側邊欄最底部的頁尾區塊新增「💬 問題反饋」長駐按鈕，不論選單如何捲動，該按鈕皆固定在底部左下角。同時配合 CSS 過渡效果與 JavaScript `switchView` 整合，實現滑順的 hover 動態效果與當前頁面高亮回饋，極大提昇頁面整潔度與使用者體驗。

#### 首頁終端機分頁公告看板（P0）
- **[新增] 終端機卡片分頁導覽**：重構首頁線上動作日誌（Terminal Card），新增「🖥️ 系統動作紀錄」與「📢 系統公告看板」分頁切換 Tabs，並配合彈性佈局（Flexbox）確保分頁在高度上完美自適應。
- **[新增] 後端公告持久化與發佈 API**：新增 `GET /api/system/announcements` 與 `POST /api/system/announcements` API。公告內容以 JSON 形式持久化儲存於後端，發佈 API 限定管理員權限調用。
- **[優化] 公告極客終端渲染與管理發佈區塊**：公告面板採用一貫的黑客代碼風格進行排版，當管理員帳號登入時，公告分頁底部將自動解鎖「公告發佈輸入框」，支援一鍵發佈與列表自動重載，給予極佳的維護體驗。
...
- **[優化] 獨立介紹頁導覽與按鈕精簡**：移除獨立介紹頁中的「服務定價」與「接案中樞」連結，並徹底刪除登入後的「打開控制台」按鈕，僅保留登入功能與登入狀態展示。
- **[優化] 主控制台專案去介紹頁化**：
  - 徹底移除主專案中的 `intro.html` 與 `pricing.html` 視圖元件與 `/introduce`、`/price` 路由對應，並移除了 `gigs.html` 中未登入的頂部導覽列。
  - 將側邊欄 Logo 點擊跳轉行為修改為回到後台首頁 (`/main`)。
  - 實作更嚴格的後台 Route Guard 路由防禦。當未登入使用者造訪系統時，強制重導向至 `/login` 並彈出不可關閉、無法透過點擊背景或關閉按鈕繞過的登入遮罩，落實純後台控制台專案的安全控制。
- **[新增] 獨立介紹頁 Tailscale 獨立節點與專屬免連接埠通道**：在介紹頁 LXC 103 容器內安裝並啟用 Tailscale，使其加入網路獲得獨立域名 `https://axis-intro-lxc.tail9f124e.ts.net/`。同時在容器內配置 `tailscale serve`，將對外 HTTPS 流量代理至本地 8080 端口，實現主控台與介紹頁在網路、域名、連接埠與代碼上的 100% 物理與邏輯隔離。
- **[新增] 獨立介紹頁用戶資料庫持久化與文字優化**：於介紹頁專案內實作 SQLite 資料庫 `users.db` 與 `users` 用戶資料表，對註冊密碼進行 SHA-256 安全加鹽雜湊（Salted Hashing）防禦，落實註冊用戶持久化儲存與安全驗證；同時將全站「登入系統」字樣全數更名為「登入」。

#### 系統日誌與個資保護（P0）
- **[修復] 系統日誌訪客個資免遮蔽**：修改 `app/routers/system.py` 中的 `mask_name` 函數，針對以 `Guest` 開頭的帳號新增例外規則。使普通會員在首頁的系統日誌列表中，可以直接讀取訪客發案時留下的聯絡資訊（例如：`Guest (聯絡資訊)`），避免其被遮蔽為 `G*******************`，確保接案聯絡管道暢通。
- **[優化] 系統日誌終端機卡片視覺與高度對齊**：重構 `dashboard.html` 與 `index.css`，將 System Logs 終端機卡片改為與全站風格一致之毛玻璃擬態面板，移除內部區域的原生純黑及深灰底色，並將輸入欄與篩選框優化為全站統一的 `.t-input` 樣式。同時將卡片高度限制為 `480px`，使其與右側前三個區塊（儲存配額、多元宇宙伺服器、基礎核心服務）完美水平對齊，並啟用內部滾動，徹底消除原有堆疊感與過度拉伸的空白。
- **[修復] 麥塊外部連線 IP 複製功能**：修改 `app/routers/minecraft.py` 與 `multiverse.js`，允許普通會員在多重宇宙面板點擊「複製」動態外網 IP 時，能成功複製真實的連線地址（例如：`1.168.22.44:25565`），但在畫面上依然保持遮蔽（`1.168.**.***:25565`）。如此兼顧螢幕/截圖之個資保護，同時不影響一般使用者之正常連線複製。
...
#### 首頁標題與選單項目優化（P2）
- **[優化] 動作紀錄初始化文字變更**：修改 `dashboard.html`，將 System Logs 動作紀錄中的初始化訊息由「[SYSTEM] 初始化戰情中心介面...」更換為「[SYSTEM] 初始化終端機介面...」。
- **[優化] 多元宇宙伺服器名稱變更**：修改 `dashboard.html`，將原本「遊戲伺服器 (Minecraft)」卡片標題更名為「💥多元宇宙伺服器」。
- **[優化] 側邊欄首頁選單項目更名**：修改 `index.html`，將原本側邊選單「🏘️ 戰情總覽」更名為「🏘️ 首頁」，以提升直覺性。

#### 接案中樞訪客發案與聯絡功能擴展（P0）
- **[新增] 訪客免登入發佈委託**：放寬 `POST /api/gigs` 必須登入的權限限制，改為可選認證，允許未登入訪客直接發佈委託案件。
- **[新增] 聯絡資訊欄位與強制校驗**：
  - 資料庫與 API 模型中新增 `contact` 欄位，且於 `main.py` 啟動階段自動為舊 SQLite 資料庫進行 `contact` 欄位之 Schema 遷移。
  - 後端 API 校驗若為未登入訪客提交案件，則強制作為必填項。
- **[新增] 前端聯絡表單與動態顯隱**：修改 `gigs.html` 與 `gigs.js`，若檢測到未登入狀態，即動態顯示「聯絡方式」輸入框，並要求訪客必須留下聯絡管道方得發佈。
- **[新增] 訪客聯絡資訊公開展示**：若發案人為 `Guest`，於接案中樞列表卡片發佈人欄位中自動展現其聯絡資訊（例如：`Guest (聯絡方式: xxx)`），以利接案人接洽。
- **[優化] 訪客承接按鈕隱藏**：修改 `gigs.js`，訪客狀態下，接案中樞列表不再顯示「承接委託」與「拒絕承接」按鈕，改為「登入以承接案件」引導按鈕，落實只有登入者可以承接案件之邏輯。
- **[新增] 案件清單分頁篩選（進行中 / 已完成）**：修改 `gigs.html` 與 `gigs.js`，於案件列表頂部複用定價頁頁籤樣式，實作「進行中 / 開放中」與「已完成委託」雙頁籤。搭配全域 cache 實作無延遲前端過濾渲染。
...
#### 公司介紹頁面聯絡資訊與標題更新（P2）
- **[優化] 介紹頁聯絡資訊更新**：修改 `intro.html`，更新聯絡我們區塊的電子郵件為 `tony951130@gmail.com`，技術研發總部地址更新為 `台灣 桃園市 國防理工技術研究園區`。
- **[優化] 標題簡化**：將主視覺 Hero 區塊的主標題由「絕對軸心科技」簡化為「絕對軸心」。
...
- **[新增] 刪除操作防護與二次確認**：前端 `admin.js` 新增 `deleteUser` 函數，實作刪除前的二次警告確認，且限制無法對當前管理員自身帳號進行刪除。

#### 側邊欄 Logo 導航優化（P1）
- **[優化] 側邊欄 Logo 導航跳轉**：修改 `index.html`，點擊側邊欄最上方的 Logo 與系統標題「ABSOLUTE AXIS」時，自動呼叫單頁路由機制跳轉回 `/introduce` 服務介紹頁面，並加入點擊手勢與防選取樣式以提升點選體驗。
...
- **[新增] 登入狀態顯示與控制台跳轉**：於已登入狀態下，將「登入控制台」與「立即體驗」等按鈕，替換為「使用者頭像」與「打開控制台」按鈕。
- **[新增] 登入頭像同步與面板操作**：於 `app.js` 的 `boot()` 函數中動態設定頭像 URL，並連結全域的 Popover 選單，支援直接在宣傳頁面一鍵點擊頭像展開設定選單，或點擊「打開控制台」直接跳轉至後台 `/main` 儀表板。

#### 路由與權限驗證邏輯 Bug 修復（P0）
- **[修復] 未登入使用者存取對外公開頁面遭攔截異常**：修改 `app.js` 的 `boot()` 函數中的路由守衛邏輯，擴展免驗證白名單以包含 `intro`、`pricing`、`gigs` 與 `login` 視圖，排除接案中樞頁面重定向與彈窗的異常。
- **[修復] 直接訪問根路徑 `/` 與網址列重定向**：修復直接訪問 `/` 時的 URL 處理邏輯，使其僅重導向至 `/introduce` 且不會錯誤觸發登入彈窗。
- **[修復] 單頁導航時導覽列登入狀態未同步更新**：將導覽列狀態的顯隱邏輯封裝至獨立函數 `updateNavActionsState()`，並在 `switchView` 視圖切換函數尾端調用，確保透過 SPA 單頁導航切換回首頁或定價頁時，導覽列能即時同步正確的已登入/未登入樣式。
